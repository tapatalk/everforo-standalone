...
//WriteLog::logfilename(message, arrayinfo);
...
class WriteLog
{
     ...
     * @var string
     */
//    private static $fileName = base_path('storage/');
    ...
    private static function createLogger($name)
    {
        if (empty(self::$loggers[$name])) {
            ...
            // 日志文件目录
            $fileName       = base_path("storage/logs/{$name}/log.log" );
            // 日志保存时间
            ...
            // 日志文件相关操作
            $handler   = new RotatingFileHandler("$fileName", $maxFiles, $level, true, $filePermission);
            // 日志格式
