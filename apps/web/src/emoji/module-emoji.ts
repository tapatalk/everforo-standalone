...
class ShortNameEmoji extends Module {
  constructor(quill: Quill, options: any) {
    ...
    this.isWhiteSpace = function(ch: string){
    ...
    quill.keyboard.addBinding({
      ...
      shiftKey: true,
    } as any, this.triggerPicker.bind(this));
    ...
    quill.keyboard.addBinding({
      key: 39,  // ArrowRight
      collapsed: true
    } as any, this.handleArrow.bind(this));
  ...
  triggerPicker(range: any, context: any) {
    if (this.open) return true;
    if (range.length > 0) {
      this.quill.deleteText(range.index, range.length, (Quill as any).sources.USER);
    }

    this.quill.insertText(range.index, ":", "emoji-shortname", (Quill as any).sources.USER);
  ...
  renderCompletions(emojis: any) {
    ...
    const handler = (i: number, emoji: any) => (event: any) => {
  ...
  close(value: any, trailingDelete = 0) {
    this.quill.enable();
    this.container.style.display = "none";
    ...
    if (value) {
      this.quill.deleteText(this.atIndex, this.query.length + 1 + trailingDelete, (Quill as any).sources.USER);
      this.quill.insertEmbed(this.atIndex, 'emoji', value, (Quill as any).sources.USER);
...
function makeElement(tag: string, attrs: any, ...children: any[]) {
